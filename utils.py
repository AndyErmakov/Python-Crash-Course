...
def unzip_with_7z(zip_file_path, dest_path, password):
    ...
    if os_type == 'Darwin':  # macOS /usr/local/bin/7z
        result = subprocess.run(['/usr/local/bin/7z', 'x', '-p' + password, '-o' + dest_path, zip_file_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
